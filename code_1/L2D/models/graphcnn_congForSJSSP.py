...
class GraphCNN(nn.Module):
    ...
    def next_layer_eps(self, h, layer, padded_neighbor_list=None, Adj_block=None):
        ...
        h = self.layer_norms[layer](pooled_rep)
        h = F.relu(h)
        return h
    ...
    def forward(self, x, graph_pool, padded_nei, adj):
        ...
        for layer in range(self.num_layers-1):
            if self.neighbor_pooling_type == "max" and self.learn_eps:
                h = self.next_layer_eps(h, layer, padded_neighbor_list=padded_nei)
            elif not self.neighbor_pooling_type == "max" and self.learn_eps:
                h = self.next_layer_eps(h, layer, Adj_block=adj)
            elif self.neighbor_pooling_type == "max" and not self.learn_eps:
                h = self.next_layer(h, layer, padded_neighbor_list=padded_nei)
